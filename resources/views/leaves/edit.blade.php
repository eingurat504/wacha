...
@section('content')
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a
                            href="{{ route('leaves.index') }}"> {{  date_diff(date_create($leave->start_date), date_create($leave->end_date))->format('%R%a')  }} Days</a>
                </li>
    ...
    <div class="row">
        <div class="col-lg-6 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('leaves.update', $leave->id) }}" class="pt-3">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="end_date" id="end_date" required
                                   ...
                                   value="{{ old('end_date', $leave->end_date) }}" placeholder="{{ __('End Date') }}"/>
                            @error('end_date')
                            <span class="invalid-feedback" role="alert">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group">
                            <textarea name="description"
                                      ...
                                      placeholder="{{ __('Description') }}">{{ trim(old('description' , $leave->description) ) }}
                            </textarea>
                            @error('description')
                            <span class="invalid-feedback" role="alert">{{ $message }}</span>
                            @enderror
